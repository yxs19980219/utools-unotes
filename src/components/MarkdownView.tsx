...
const HEADING_CLASS = [
  'text-[1.6rem] font-bold leading-snug', // h1（与编辑器装饰对齐）
  'text-[1.35rem] font-semibold leading-snug', // h2
  'text-[1.15rem] font-semibold', // h3
  'text-[1rem] font-semibold text-muted-foreground', // h4
]
...
export default function MarkdownView({
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 text-[1rem] leading-relaxed', className)}>
      {blocks.map((b, i) => {
        const key = `b${i}`
        switch (b.kind) {
          case 'heading':
